...
class InceptionI3d(snt.AbstractModule):
  ...
  def _build(self, inputs, is_training, dropout_keep_prob=1.0):
    ...
    net = Unit3D(output_channels=64, kernel_shape=[7, 7, 7],
                 stride=[1, 2, 2], name=end_point)(net, is_training=is_training) #Default stride=[(2), 2, 2]
    ...
    net = tf.nn.max_pool3d(net, ksize=[1, 3, 3, 3, 1], strides=[1, 1, 2, 2, 1], #Default strides=[1, (2), 2, 2, 1]
                           padding=snt.SAME, name=end_point)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
    ...
    net = tf.nn.max_pool3d(net, ksize=[1, 2, 2, 2, 1], strides=[1, 1, 2, 2, 1], #Default strides=[1, (2), 2, 2, 1],
                           padding=snt.SAME, name=end_point)
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
    ...
    end_point = 'Logits_I3D'
    with tf.variable_scope(end_point):
      net = tf.nn.avg_pool3d(net, ksize=[1, 1, 7, 7, 1], #Default: ksize=[1, (2), 7, 7, 1]
                              strides=[1, 1, 1, 1, 1], padding=snt.VALID)
      net = tf.nn.dropout(net, dropout_keep_prob)
      logits = Unit3D(output_channels=1024,
                      kernel_shape=[1, 1, 1],
                      activation_fn=None,
    ...
    end_point = 'Logits_LSTM'
    with tf.variable_scope(end_point):
        cells = []
        for _ in range(self._num_layers_lstm):
            cell = tf.nn.rnn_cell.LSTMCell(num_units=128, forget_bias=1.0, state_is_tuple=True)
            cells.append(cell)
        multi_cell = tf.nn.rnn_cell.MultiRNNCell(cells, state_is_tuple=True)
        outputs, final_state = tf.nn.dynamic_rnn(multi_cell, logits, dtype=tf.float32)
    logits = tf.layers.dense(outputs, self._num_classes)
    outputs = tf.reduce_mean(logits, axis=1)
    end_points[end_point] = outputs
    print(end_points)
    if self._final_endpoint == end_point: return outputs, end_points
